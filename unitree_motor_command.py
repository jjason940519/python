...
class unitree_communication(object):
    ...
    def inital_check(self):
        for motor in self.motors:
            if motor.inital_position_max >= motor.data.q and motor.data.q >= motor.inital_position_min:
                ...
                motor.cmd.q    = motor.inital_position_check
                motor.cmd.dq   = 0
                motor.cmd.kp   = 6
                motor.cmd.kd   = 0
                motor.cmd.tau  = 0
    ...
    def calibrate_all_motor(self):
        ...
        for motor in self.motors:
            self.serial.sendRecv(motor.cmd, motor.data)
            time.sleep(6)  
        print("motor calibrate finish")

    def position_force_velocity_cmd(self,motor_number=0,torque=0,kp=0,kd=0,position=0,velocity=0,check_flag=True):
        for motor in self.motors:
            if check_flag == True:
                if motor.id == motor_number:
                    if  motor.max_position>=motor.data.q>=motor.min_position and motor.inital_success:
                        motor.cmd.mode = queryMotorMode(MotorType.GO_M8010_6,MotorMode.FOC)
...
class unitree_motor(object):                                                                                  
    def __init__(self,motor_id = 0,MAX_degree = 0,MIN_degree = 0, inital_position_check = 0):
        ...
        self.inital_position_check = inital_position_check
        self.inital_position_max = inital_position_check + 0.6
        self.inital_position_min = inital_position_check - 0.6
        self.inital_success = False
